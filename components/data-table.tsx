 ...
 * Contains the query string for searching the data table.
 @constant {number} currentPage - Tracks the current page in the paginated data table.
 */
  @constant {string} sortColumn - Stores the column key for the current sorting criteria.
  pageSize = 10,
  @constant {string} sortDirection - Indicates the sorting direction ('asc' for ascending, 'desc' for descending).
  searchable = false,
  @constant {string} searchQuery - Contains the query string for searching the data table.
  /** @constant filteredData
 * Memoized value of data filtered based on search query and sort criteria.
 @constant {Array} filteredData - Memoized value of data filtered based on search query and sort criteria.
 */
  searchFields
}: DataTableProps<T>) {
...
@constant {Array} paginatedData - Memoized value of data divided into pages for pagination.

    if (sortColumn) {
      result.sort((a, b) => {
        /** @constant paginatedData
 @constant {number} totalPages - Calculates the total number of pages based on the filtered data length and page size.
 * Memoized value of data divided into pages for pagination.
 */
        @function handleSort - Handles sorting when a sortable column header is clicked.
 ...
 */
        if (aVal > bVal) return sortDirection === 'asc' ? 1 : -1;
        @function handlePageChange - Handles pagination by changing the current page within bounds.
        return 0;
      /** @function handleSort
